...
# betadisp wu by loc
beta_disp_fltr_loc <- betadisper(wunifrac_distances_fltr, metadata_fltr$Location)
anova_beta_disp_fltr <- anova(beta_disp_fltr_loc)
anova_beta_disp_fltr
...
#bet disp depth
beta_disp_u_d <- betadisper(unifrac_distances, metadata$Depths)
anova_beta_disp_u_d <- anova(beta_disp_u_d)
...
# Extract PERMDISP
p_dispersion_u <- anova(beta_disp_u)$`Pr(>F)`[1]
p_dispersion_wu <- anova(beta_disp_fltr)$`Pr(>F)`[1]

# join
tableS10_final <- tableS10_permanova_u %>%
  mutate(Dispersion_p = ifelse(Factor == "Location", p_dispersion_u, NA)) %>%
  mutate(Dispersion_p = ifelse(Factor == "Depths", p_dispersion_wu, NA)) %>%
  mutate(across(where(is.numeric), ~ round(., 4)))

#show
tableS10_final
#save
write.csv(tableS10_final, "Tables/interior_mangroves/Supplementary_Table_S10_Interior_BetaDisper.csv", row.names = FALSE)
```

```{r}
library(dplyr)

# extract unweighted UniFrac 
tab_u <- as.data.frame(adonis2_unifrac) %>%
  mutate(Factor = rownames(.), Metric = "Unweighted UniFrac") %>%
  dplyr::select(Metric, Factor, Df, R2, F.Model = F, p_value = `Pr(>F)`) %>%
  filter(!Factor %in% c("Total", "Residual")) %>%
  mutate(Dispersion_p = case_when(
    Factor == "Location" ~ anova(beta_disp_u)$`Pr(>F)`[1],
    Factor == "Depths" ~ anova(beta_disp_u_d)$`Pr(>F)`[1],
    TRUE ~ NA_real_
  ))

# extract weighted UniFrac 
tab_wu <- as.data.frame(adonis2_wunifrac_fltr) %>%
  mutate(Factor = rownames(.), Metric = "Weighted UniFrac") %>%
  dplyr::select(Metric, Factor, Df, R2, F.Model = F, p_value = `Pr(>F)`) %>%
  filter(!Factor %in% c("Total", "Residual")) %>%
  mutate(Dispersion_p = case_when(
    Factor == "Depths_fltr" ~ anova(beta_disp_fltr)$`Pr(>F)`[1],
    Factor == "Location" ~ anova(beta_disp_fltr_loc)$`Pr(>F)`[1],
    TRUE ~ NA_real_
  ))

# join
tableS10_final <- bind_rows(tab_u, tab_wu) %>%
  mutate(across(where(is.numeric), ~ round(., 4)))

# Guardar
write.csv(tableS10_final, "Tables/interior_mangroves/Supplementary_Table_S10_Interior_Mangroves_Beta.csv", row.names = FALSE)
```
