...
tableS9_int_posthoc <- bind_rows(
  ...
  as.data.frame(cld_resultsq1) %>% mutate(Metric = "q1"),
  as.data.frame(cld_resultsq2) %>% mutate(Metric = "q2"),
  as.data.frame(cld_resultsqf) %>% mutate(Metric = "qf")) %>%
  dplyr::select(Metric, Depths, emmean, SE, lower.CL, upper.CL, Significance = .group) %>%
...
# save
write.csv(tableS9_int_models, "Tables/interior_mangroves/Supplementary_Table_S9_Interior_LMM.csv", row.names = FALSE)
write.csv(tableS9_int_posthoc, "Tables/interior_mangroves/Supplementary_Table_S9_Interior_PostHoc.csv", row.names = FALSE)
```
