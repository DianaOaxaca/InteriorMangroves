...
```{r}
#load data
physeq <- readRDS("rds/interior_impaired/physeq.rds")
```

```{r}
# Convert Mangrove_system to factor
sample_data(physeq)$Mangrove_system <- as.factor(sample_data(physeq)$Mangrove_system)
```
...
core_list <- list()
locations <- levels(sample_data(physeq)$Mangrove_system)

# Calculate core
for (loc in locations) {
  # Filter samples by mangrove system
  physeq_loc <- prune_samples(sample_data(physeq)$Mangrove_system == loc, physeq)
...
# Impaired Términos Lagoon did not present unique ASV, therefore the bar representing this mangrove is not draw

# queries_list <- append(queries_list, list(
#   list(query = intersects, params = list("Impaired Términos Lagoon"), color = "gray40",
#        active = TRUE, query.name = "Impaired Términos Lagoon")))
...
```{r}
pdf("Figures/interior_impaired/core_upset.pdf", width = 5.8, height = 4)
upset_plot
dev.off()
```
...
# Iterate on each Mangrove_system
for (loc in names(taxonomy_unique)) {
  if (!is.null(taxonomy_unique[[loc]])) {
...
```{r}
# save data.frame
write.csv(taxonomy_unique_df, file = "Tables/interior_impaired/unique_asvs_taxonomy.csv", row.names = FALSE)
```
...
```{r}
ampv_heatmap_abundances_fam_core <- amp_heatmap(av2,
            group_by = "Mangrove_system",
            facet_by = "Mangrove_type",
            plot_values = TRUE,
            tax_show = 40,
...
ampv_heatmap_abundances_fam_core
```

```{r}
# save
ggsave("Figures/interior_impaired/Core_RelativeAbundance_heatmap_fam.pdf",
       ampv_heatmap_abundances_fam_core, bg='transparent', 
       width = 12, height = 8)
```
...
ampv_heatmap_abundances_phylum_core <- amp_heatmap(av2,
            ...
            tax_show = 40,
            showRemainingTaxa = TRUE,
            #tax_aggregate = c("Phylum", "Class"),
            tax_aggregate = c("Phylum"),
            plot_colorscale = "log10",
            plot_values_size = 4,
            min_abundance = 0.001,
            round = 2,
...
```{r}
# save
ggsave("Figures/interior_impaired/Core_RelativeAbundance_heatmap_phylum.pdf",
...
```{r}
saveRDS(upset_plot, "rds/interior_impaired/upset_plot.rds")
```
